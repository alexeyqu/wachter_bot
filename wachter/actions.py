        ...
        with session_scope() as sess:
            ...
            if chat.on_kick_message.lower() not in ["false", "0"]:
                ...
                if (job.context['chat_id'] == constants.RH_CHAT_ID):
                    message_markdown = mention_markdown(
                        bot, job.context['chat_id'], job.context['user_id'], random.choice(constants.RH_kick_messages))
...
def on_start_command(bot: Bot, update: Update, user_data: dict):
    ...
    keyboard = [[InlineKeyboardButton(chat['title'],
                                      callback_data=json.dumps({'chat_id': chat['id'], 'action': constants.Actions.select_chat}))]
...
def on_button_click(bot: Bot, update: Update, user_data: dict):
    ...
    if data['action'] == constants.Actions.select_chat:
        ...
        keyboard = [
            [InlineKeyboardButton('Изменить таймаут кика', callback_data=json.dumps(
                {'chat_id': selected_chat_id, 'action': constants.Actions.set_kick_timeout}))],
            [InlineKeyboardButton('Изменить сообщение при входе в чат', callback_data=json.dumps(
                {'chat_id': selected_chat_id, 'action': constants.Actions.set_on_new_chat_member_message_response}))],
            [InlineKeyboardButton('Изменить сообщение при перезаходе в чат', callback_data=json.dumps(
                {'chat_id': selected_chat_id, 'action': constants.Actions.set_on_known_new_chat_member_message_response}))],
            [InlineKeyboardButton('Изменить сообщение после успешного представления', callback_data=json.dumps(
                {'chat_id': selected_chat_id, 'action': constants.Actions.set_on_successful_introducion_response}))],
            [InlineKeyboardButton('Изменить сообщение напоминания', callback_data=json.dumps(
                {'chat_id': selected_chat_id, 'action': constants.Actions.set_notify_message}))],
            [InlineKeyboardButton('Изменить сообщение после кика', callback_data=json.dumps(
                {'chat_id': selected_chat_id, 'action': constants.Actions.set_on_kick_message}))],
            [InlineKeyboardButton('Изменить regex для фильтра сообщений', callback_data=json.dumps(
                {'chat_id': selected_chat_id, 'action': constants.Actions.set_regex_filter}))],
            [InlineKeyboardButton("Изменить фильтрацию только для новых пользователей", callback_data=json.dumps(
                {'chat_id': selected_chat_id, 'action': constants.Actions.set_filter_only_new_users}
            ))],
            [InlineKeyboardButton('Получить текущие настройки', callback_data=json.dumps(
                {'chat_id': selected_chat_id, 'action': constants.Actions.get_current_settings}))]
        ]
    ...
    elif data['action'] in [constants.Actions.set_on_new_chat_member_message_response,
        ...
        keyboard = [
            [InlineKeyboardButton('К настройке чата', callback_data=json.dumps(
                {'chat_id': data['chat_id'], 'action': constants.Actions.select_chat})),
...
def on_message(bot: Bot, update: Update, user_data: dict, job_queue: JobQueue):
    ...
    if chat_id < 0:
        ...
        if action == constants.Actions.set_kick_timeout:
            ...
            keyboard = [
                [InlineKeyboardButton('К настройке чата', callback_data=json.dumps(
                    {'chat_id': chat_id, 'action': constants.Actions.select_chat})),
                InlineKeyboardButton('К списку чатов', callback_data=json.dumps(
                    {'action': constants.Actions.start_select_chat}))],
            ]
        ...
        elif action in [constants.Actions.set_on_new_chat_member_message_response,
                        constants.Actions.set_notify_message,
                        constants.Actions.set_on_known_new_chat_member_message_response,
                        constants.Actions.set_on_successful_introducion_response,
                        constants.Actions.set_on_kick_message,
            ...
            with session_scope() as sess:
                ...
                if action == constants.Actions.set_on_known_new_chat_member_message_response:
                    chat = Chat(
