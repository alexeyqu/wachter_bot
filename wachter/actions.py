    ...
    try:
        ...
        with session_scope() as sess:
            ...
            if chat.on_kick_message.lower() not in ["false", "0"]:
                ...
                if job.context["chat_id"] == constants.constants.Actions.RH_CHAT_ID:
                    message_markdown = mention_markdown(
                        bot,
                        job.context["chat_id"],
                        job.context["user_id"],
                        random.choice(constants.constants.Actions.RH_kick_messages),
                    )
                bot.send_message(
                    job.context["chat_id"],
    ...
    if data["action"] == constants.Actions.select_chat:
        ...
        keyboard = [
            [
                ...
                InlineKeyboardButton(
                    "К списку чатов",
                    callback_data=json.dumps({"action": constants.Actions.start_select_chat}),
                ),
            ],
        ]
