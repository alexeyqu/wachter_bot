...
from src.handlers.group.group_handler import delete_message, on_kick_timeout, on_notify_timeout
            ...
            for job in self.job_queue.jobs():
                jobs.append({
                    ...
                    'context': {
                        ...
                        "timeout": datetime.timestamp(job.context.get("timeout")) if job.context.get("timeout") else None,
                    }
                })
                ...
                for job in jobs:
                    self.job_queue.run_once(
                        ...
                        context={
                            ...
                            "message_id": job["context"].get("message_id"),
                            "creation_time": job["context"].get("creation_time"),
                            "timeout": datetime.fromtimestamp(job["context"].get("timeout")) if job["context"].get("timeout") else None,
                        },
                    )
